(function () {
  ...
  function renderList(game) {
    ...
    const gameMuseums = museums.filter(m => m.game === game).sort((a, b) => a.num - b.num);
  ...
  function updateSidebarItem(game, num) {
    const museum = museums.find(m => m.game === game && m.num === num);
    if (!museum) return;
    const item = document.querySelector(`.museum-item[data-id="${museum.id}"]`);
    if (!item) return;

    const v = isVisited(museum);
    item.classList.toggle('visited', v);
    item.querySelector('input[type="checkbox"]').checked = v;
  }
